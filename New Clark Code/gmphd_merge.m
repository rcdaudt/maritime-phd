...
while(~isempty(I))
    ...
    HypN(l_count).tag = Hyp(j).tag;
    HypN(l_count).entry = Hyp(j).entry;
    HypN(l_count).status = Hyp(j).status;
    if HypN(l_count).wk > .8 %THRESHOLD TO CONFRIM TRACK IN TRACK MANAGEMENT
        HypN(l_count).status = 1;
    end
